...
"""
import unittest
from tests.utils import gen_len
...
class CombinationsTestCase(unittest.TestCase):
    ...
    def test_combinations_one_word(self):
        ...
        res = thechronic.combine(num_words=1)
        # import pdb; pdb.set_trace()
        self.assertEqual(gen_len(res), 3)

        self.assertCountEqual(res, self.words1)

        # make sure that num_words defaults to 1
        res = thechronic.combine()
        self.assertEqual(gen_len(res), 3)
    ...
    def test_combinations_multiple_words_single_source(self):
        ...
        res = thechronic.combine(num_words=2)
        self.assertEqual(gen_len(res), 9)
        self.assertCountEqual(res, expected_result)

        expected_result = product(self.words1, repeat=3)
        res = thechronic.combine(num_words=3)
        self.assertEqual(gen_len(res), 27)
        self.assertCountEqual(res, expected_result)
    ...
    def test_combinations_multiple_words_multiple_sources(self):
        ...
        res = thechronic.combine(num_words=2)
        self.assertEqual(gen_len(res), 36)
        self.assertCountEqual(res, expected_result)

        expected_result = product(self.words1 + self.words2, repeat=3)
        res = thechronic.combine(num_words=3)
        self.assertEqual(gen_len(res), 216)
        self.assertCountEqual(res, expected_result)
